                ...
                forecast.map(item => {
                    return(
                        <div className="forecast" key={item.EpochDate}>
                            ...
                            <img alt='icon' src={`https://developer.accuweather.com/sites/default/files/${forecast[0].Day.Icon<10?'0'+forecast[0].Day.Icon:forecast[0].Day.Icon}-s.png`} />
                            <h3>{item.Date.substring(0,10).split('-').reverse().join('-')}</h3>
                            <h5>{item.Temperature.Maximum.Value}<span>&#176;</span>-{item.Temperature.Minimum.Value}<span>&#176;</span></h5>
                        </div>
                    )
                })
            }
